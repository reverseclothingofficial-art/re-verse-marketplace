...
interface BrandOrder {
  ...
  returnStatus: "pending" | "accepted" | "rejected" | "completed"
...
interface Order {
  ...
  status: "placed" | "processing" | "completed" | "cancelled" | "failed"
...
export default function OrdersPage() {
  ...
  // 2. Fetch orders function with cookieValue in dependencies
  const fetchOrders = useCallback(
    async (page = 1, append = false) => {
      try {
        ...
        const response = await fetch(`${api}/api/order/my/orders?page=${page}&limit=10`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${cookieValue}`,
  ...
  // 3. Load user profile once on mount
  useEffect(() => {
    if (cookieValue) {
      userProfile()
    }
  ...
  // Only fetch orders when cookieValue is available
  useEffect(() => {
    if (cookieValue) {
      fetchOrders()
    }
  ...
  // 5. Scroll handler with proper dependencies
  ...
  // 6. Attach scroll listener safely (guard for SSR)
  useEffect(() => {
  ...
  // 7. Clear message timeout with cleanup to avoid memory leaks
  useEffect(() => {
  ...
  const cancelOrder = async (orderId: string) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((order) => (order._id === orderId ? { ...order, status: "cancelled" as const } : order))
      )

      setMessage({ type: "success", text: "Order cancelled successfully!" })
    } catch (error) {
      setMessage({ type: "error", text: "Failed to cancel order. Please try again." })
    } finally {
  ...
  const canCancelOrder = (order: Order) => {
    return (
      order.status === "placed" && !order.brandOrders.some((bo) => bo.status === "shipped" || bo.status === "delivered")
    )
  }
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {currentPage >= totalPages && orders.length > 0 && (
              <Card className="shadow-lg border-0 rounded-2xl">
                <CardContent className="p-6 text-center">
                  <CheckCircle className="h-8 w-8 text-[#33BF69] mx-auto mb-2" />
                  <p className="text-[#455A64]">You've reached the end of your orders</p>
                </CardContent>
              </Card>
            )}
          </div>
